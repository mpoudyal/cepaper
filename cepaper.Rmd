...
- *sg_com* (=**120**): Average safeguard compensation expected to be spent per household <sup id="n1">[1](#f1)</sup>
...
comp.plot1 <- ggplot(comp.df.melt[c(1,5,6),],
  ...
  scale_x_discrete(limits=c("npv_oc_all","sg_com","cv_com",""), 
                   labels = c("Median HH opportunity \ncosts (NPV)",
                              "Average projected spend \non compensation per HH",
                              "Average value of \ncompensation per HH",""))
...
comp.plot1i <- ggplot(comp.df.melt[c(12,1,5,6),], 
                     aes(variable, value, fill=variable, label=value)) + 
  geom_col(width = 0.5) + theme_bw() + 
