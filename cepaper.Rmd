...
Some of the analyses and production of figures might not fully replicate under different versions of R and the packages than we've used here (see section *Opportunity cost estimates from the CE model (NPV)* below for one such example). Version details for R and all packages used (this list doesn't include dependencies that load with these core packages) in this analysis is listed below:
...
## Basic PCA plot (not included in the publication)
...
## PCA biplot (Fig 2-a) [requires 'ggplot2' package, load if not loaded already]
require(ggplot2)
...
## PCA scatter plot for all sites with convex hulls around scatter plot for each site (Fig 2-b)
...
## Combining two plots to create a panel (Figure 2) [requires 'grid' package, load if not loaded already]
require(grid)

grid.draw(cbind(ggplotGrob(pcplot1), ggplotGrob(pcplot2), size="last"))

## output as PDF
pdf("fig2.pdf", width = 11, height = 5.5)
grid.draw(cbind(ggplotGrob(pcplot1), ggplotGrob(pcplot2), size="last"))
dev.off()
...
We use the data on landholding and ways respondent households gained access to their plots of land, obtained from the first phase of the survey (see main text and SI, and <b id="n7">[Poudyal et al. (2016)](#r1)</b>) to produce **SI Figure 1** that summarises the various ways households in our study sites gained access to their plots of land. Steps involved in producing **SI Figure 1** with codes below.
...
##pdf output
pdf("si_fig_1.pdf", width=11, height=8)
print(land.plot)
dev.off()
...
##For R versions up to R 3.3.x (duplicated levels in factor only produce warning, not error) and code below works. Uncomment to use for R versions up to 3.3.x.
...
##For R versions R 3.4.x and above (duplicated levels in factor produce error, hence unique levels required). Use code below for R 3.4.x and above.
paper.model.cons.wta$coefficient2 <- factor(c("asc", "Instalment 2", "Instalment 3", "Rice project", "Teviala 1", "New PA\nwith safeguard",
...
## Combining two plots to create a panel (Figure 3)
...
# Plotting annualised equivalent of opportunity costs against total annual household income (Figure 4)
oc_hhinc_plot <- ggplot(oc.df.sub1.melt, aes(x=usd, y=value,
            ...
            scale_colour_discrete(name="Time frame (t) and discount rate (r)",
                 breaks=c("prop3","prop2","prop1"),
                                labels=c(" t = 60 years, r = 5%",
                                         " t = 60 years, r = 2.5%",
                                         " t = 60 years, r = 0.001%")) + 
            theme_bw() +
...
## output plot as pdf
pdf("fig4.pdf", width = 11, height = 8)
print(oc_hhinc_plot)
dev.off()
...
pdf("fig5.pdf", width = 11, height = 10) 
...
##plot output as pdf
pdf("si_fig2.pdf", width = 11, height = 8)
print(occv.plot)
dev.off()
...
## plot output as pdf
pdf("si_fig_5.pdf", width = 8, height = 8)
print(cazpop.plot)
dev.off()
...
<b id="f5">5.</b> Source: Interagency Working Group on Social Cost of Greenhouse Gases, United States Government (2016) - "Technical Support Document: Technical Update of the Social Cost of Carbon for Regulatory Impact Analysis Under Executive Order 12866, August 2016". [↩︎](#n5)
