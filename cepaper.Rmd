...
date: "2 January 2017"
latest revision: "25 February 2017"
...
All data used in the avalyses and in producing the figures outlined here are included in this repository. To fully replicate the analyses and produce the figures (copies of which are included in the repository as pdf files), clone/download the repository and follow the steps outlined in this document.
...
poverty.pca.set.nice <- c("Number of rooms", 
                          ...
                          "Access to light", 
                          "Radio")
## PCA [requires 'polycor' and 'psych' packages, load if not loaded already by uncommenting the lines below]
# require(polycor)
# require(psych)
...
## [requires 'dplyr' package, load if not loaded already by uncommenting the line below]
# require(dplyr)
...
## PCA biplot (SI Figure S1-a) [requires 'ggplot2' package, load if not loaded already by uncommenting the line below]
# require(ggplot2)
...
## Combining two plots to create a panel (SI Figure S1) [requires 'grid' package, load if not loaded already by uncommenting the line below]
# require(grid)

grid.draw(cbind(ggplotGrob(pcplot1), ggplotGrob(pcplot2), size="last"))

## output as PDF
pdf("si_fig_s1.pdf", width = 11, height = 5.5)
grid.draw(cbind(ggplotGrob(pcplot1), ggplotGrob(pcplot2), size="last"))
dev.off()
...
## without plot_type grouping (but keeping site groupings) [requires 'dplyr' package, load if not loaded already by uncommenting the line below]
# require(dplyr)
...
## Plotting land data, grouped by sites [requires 'ggplot2' package, load if not loaded already by uncommenting the line below]
# require(ggplot2)
...
##pdf output
pdf("si_fig_s2.pdf", width=11, height=8)
print(land.plot)
dev.off()
...
## Ariary to USD conversion (see main text for details on exchange rates and data source)
usd.conversion <- 1e6/2702
...
## [requires 'msm' package, load if not loaded already by uncommenting the line below]
# require(msm)
...
oc.df$usd <- round(oc.df$tot_hh_inc/2702)
...
## function for calculating EAA factor (this when multiplied by the NPV OC estimate gives the annualised equivalent) [Load this function now if not already loaded]
eaa <- function(t, r) {r/(1-(1+r)^-t)}
...
- *caz_tot_hh* (=**9837**): Total number of households estimated to be inhabiting in/around CAZ protected area (2km buffer)
...
##function for extracting total number of households with NPV OC greater than a given threshold in CAZ [Load the function below now if not already loaded]
pap_hh <- function(i, c) {round((sum(c>i)/196)*9837)}
...
hh.plot1 <- hh.plot1 + 
  geom_text(position = position_dodge(0.9), vjust=-0.25) + 
  annotate("text", x = 0.5, y = 6400, label = "b)", size = 9)
...
##plot output as pdf
pdf("fig5.pdf", width = 11, height = 8)
print(occv.plot)
dev.off()
...
## plot output as pdf
pdf("si_fig_s5.pdf", width = 8, height = 8)
print(cazpop.plot)
dev.off()
